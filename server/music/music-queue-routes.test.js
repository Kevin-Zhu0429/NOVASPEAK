...
test("播放控制：Admin/Member/User 可用，Guest 被拒绝", async () => {
  membership.set("user-a", "cs2");
  ...
  setUser("ordinary-control", { role: "user", displayName: "普通用户" });
  membership.set("ordinary-control", "cs2");
  const ordinaryPause = await api(
    "POST",
    "/api/music/netease/channels/cs2/playback/pause",
    { paused: false }
  );
  assert.equal(ordinaryPause.status, 200);
  const ordinarySkip = await api(
    "POST",
    "/api/music/netease/channels/cs2/playback/skip"
  );
  assert.equal(ordinarySkip.status, 200);
  const ordinaryView = await api(
    "GET",
    "/api/music/netease/channels/cs2/queue"
  );
  assert.equal(ordinaryView.json.controls.canControlPlayback, true);
...
test("随机与置顶：普通用户可用、Guest 被拒，响应不泄露内部排序", async () => {
  setUser("user-a", { role: "user", displayName: "普通用户" });
  membership.set("user-a", "cs2");
...
test("DJ 过渡开关：正式账号可切换、Guest 与未登录被拒、快照透出状态", async () => {
  setUser("user-a", { role: "user", displayName: "普通用户" });
  membership.set("user-a", "cs2");
