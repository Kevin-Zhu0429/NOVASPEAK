...
test("在线成员管理通过随机 presenceId 定向发送控制，不向 snapshot 泄露真实账号 ID", async () => {
  const service = createPresenceService({ heartbeatMs: 60_000 });
  const target = new FakeConnection();
  service.addConnection(target, {}, formalUser);
  const publicTarget = service.publicMembers("user:observer")[0];
  assert.equal(typeof publicTarget.presenceId, "string");
  assert.equal(JSON.stringify(publicTarget).includes(formalUser.id), false);

  const internalTarget = service.getManagementTarget(publicTarget.presenceId);
  assert.equal(internalTarget.userId, formalUser.id);
  assert.equal(internalTarget.role, "admin");
  assert.equal(service.sendVoiceControlToPresenceMember(publicTarget.presenceId, {
    type: "voice_control", action: "force_move_channel", targetChannelId: "lobby",
  }), true);
  assert.equal(voiceControlsOf(target).at(-1).action, "force_move_channel");

  assert.equal(service.disconnectPresenceMember(publicTarget.presenceId, {
    type: "voice_control", action: "force_logout",
  }), true);
  assert.equal(voiceControlsOf(target).at(-1).action, "force_logout");
  await new Promise((resolve) => setTimeout(resolve, 80));
  assert.equal(target.closeCode, 4003);
  service.close();
});
